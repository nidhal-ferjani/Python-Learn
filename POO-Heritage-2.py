...
class SportTeam(object):
    ...
    def __init__(self,name, wins, losses, draws=0):
        ...
        if self.__class__.GAME_CAN_BE_A_TIE:
            self.draws = draws
    ...
    def stats(self):
        ...
        res = f'[{sport_type} STATS]: '

        if self.__class__.GAME_CAN_BE_A_TIE:
            res += f"{self.name} has {utils.pluralize(self.wins, 'victoire')}, {utils.pluralize(self.losses, 'défaite')} and  {utils.pluralize(self.draws, 'nul')}"
        else:
            res += f"{self.name} has {utils.pluralize(self.wins, 'victoire')} and {utils.pluralize(self.losses, 'défaite')}"
...
if __name__ == '__main__':
    ...
    print(BasketBallTeam.stats(team_11))
